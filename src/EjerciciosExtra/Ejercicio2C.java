...
public class Ejercicio2C {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(SumarDiagonal(Matriz, orden, i, j));

    }

    public static int SumarDiagonal(int Matriz[][], int orden, int fila, int colum) {
        int sumaDiagonal = 0;
        if (fila != orden - 1 || colum != orden - 1) {
            if (colum < orden) {
                if (fila == colum) {
                    sumaDiagonal = sumaDiagonal + Matriz[fila][colum] + SumarDiagonal(Matriz, orden, fila, colum + 1);
                } else {
                    sumaDiagonal = sumaDiagonal + SumarDiagonal(Matriz, orden, fila, colum + 1);
                }
            } else {
                sumaDiagonal = SumarDiagonal(Matriz, orden, fila + 1, 0);
            }
        } else {
